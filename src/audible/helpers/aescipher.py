...
def verify_salt_marker(salt_marker):
    return 1 <= len(salt_marker) <= 6 and isinstance(salt_marker, bytes)
...
class AESCipher:
    ...
    def __init__(self, password: str, *, key_size: int = 32,
        ...
        if verify_salt_marker(salt_marker):
            self.salt_marker = salt_marker
        else:
            raise ValueError('The salt_marker has wrong format.')
